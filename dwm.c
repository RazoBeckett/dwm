...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) { /* status is only drawn on selected monitor */
    tw = statusw = m->ww + stw - drawstatusbar(m, bh, stext);
  }
...
void tile(Monitor *m) {
  ...
  unsigned int gap =
      (n > 1) ? gappx : fgappx; // Choose gap based on the number of clients
  ...
  for (i = 0, my = ty = gap, c = nexttiled(m->clients); c;
       c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      h = (m->wh - my) / (MIN(n, m->nmaster) - i) - gap;
      resize(c, m->wx + gap, m->wy + my, mw - (2 * c->bw) - gap * (5 - ns) / 2,
             h - (2 * c->bw), False);
      my += HEIGHT(c) + gap;
    } else {
      h = (m->wh - ty) / (n - i) - gap;
      resize(c, m->wx + mw + gap / ns, m->wy + ty,
             m->ww - mw - (2 * c->bw) - gap * (5 - ns) / 2, h - (2 * c->bw),
             False);
      if (ty + HEIGHT(c) < m->wh)
        ty += HEIGHT(c) + gap;
    }
